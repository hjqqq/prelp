...
#define mysql_query_format(%1,%2,%3)\
    new uyshjad[%1],format(uyshjad,sizeof(uyshjad),%2,%3),mysql_query(uyshjad)
...
new names_of_month[12][15] = {"Styczeñ","Luty","Marzec","Kwiecieñ","Maj","Czerwiec","Lipiec","Sierpieñ","Wrzesieñ","PaŸdziernik","Listopad","Grudzieñ"};
...
public OnGameModeInit()
{
	...
	Crp->Notify("core", "Uruchamianie modu³ów...");
...
{    
	...
	{
      String->Format("B³¹d po³¹czenia z baz¹ danych `%s`", SQL_DB);
	  Crp->Notify("mysql", formatted);
	}
...
forward mysql_fetch_field_float(field[]);
public mysql_fetch_field_float(field[])
{
  new tmp_str[60];
  mysql_fetch_field(field, tmp_str);
...
forward mysql_fetch_field_bool(field[]);
public mysql_fetch_field_bool(field[])
{
  new tmp_str[5];
  mysql_fetch_field(field, tmp_str);
...
stock LoadGroups()
{
  ...
  String->Format("Wczytano %d grup", mysql_num_rows());
  ...
  for(new i;i<mysql_num_rows();i++)
  {
    ...
    groups[i][gColor] = mysql_fetch_field_int("colors");
    groups[i][grid] = mysql_fetch_field_int("uid");
    groups[i][type] = mysql_fetch_field_int("type");
	groups[i][bank] = mysql_fetch_field_int("bank");
	mysql_fetch_field("name", groups[i][name]);
    Iter_Add(Groups, i);	
...
public GroupVehiclesLoadQuery()
{
  ...
  cache_get_data( rows, fields, mysqlHandle);
   // -- Notification -- //
  String->Format("Wczytano %d pojazdów grup.", rows);
...
public ItemsLoadQuery()
{
	...
	cache_get_data(rows, fields, mysqlHandle);
	 // -- Notification -- //
    String->Format("Wczytano %d przedmiotów", rows);
...
public ObjectsLoadQuery()
{
   ...
   cache_get_data( rows, fields, mysqlHandle);
    // -- Notification -- //
   String->Format("Wczytano %d obiektów", rows);
...
{
	Crp->Notify("pojazdy", "Zapisywanie wszystkich pojazdów...");
    ...
    Crp->Notify("przedmioty", "Zapisywanie przedmiotów na ziemi...");
	
    Crp->Notify("j¹dro", "Zamykanie modu³ów...");
	return 1;
}
...
public BeforeLoginQuery(playerid)
{
	...
	if( rows == 1 ) 
	{
	   ...
	   format( string, sizeof(string), "Witaj %s, pod³¹czy³eœ siê do serwera PLERP.net.\n\
	                                    Dziœ jest %s, %d dzieñ w roku.\n\n\
										Aby rozpocz¹æ grê postaci¹ {DCD6BC}%s {a9c4e4}zaloguj siê podaj¹c has³o\n\
										w polu poni¿ej. Mo¿esz tak¿e skorzystaæ z dodatkowych opcji.", globaln[playerid], FormatDataForShow(gettime(), 1), getdate(tmpInt, tmpInt, tmpInt), pName[playerid] );
	...
	{
	   new string[160];
	   format( string, sizeof(string), "Postaæ {DCD6BC}%s {a9c4e4}nie istnieje w naszej bazie danych.\n\
	                                    Zmien swoj¹ postaæ lub wejdŸ na www.plerp.net, aby siê zarejestrowaæ.", pName[playerid] );
       ShowPlayerDialog(playerid, 73, DIALOG_STYLE_MSGBOX, BuildGuiCaption("PLERP.net » {AFCC70}Nie posiadasz konta"), string, "Zmieñ postaæ", "WyjdŸ" );
	}
	return 1;
}
...
public LoginQuery(playerid)
{
	...
	if( rows == 1 ) {
	   ...
	   if(block == 1)
	   {
			SendClientMessage(playerid, COLOR_RED, "Ta postaæ jest uœmiercona.");
			Kick(playerid);
			return 1;
	   }
	   ...
	   // -- welcome message -- //
	   format(msg, sizeof(msg), "{B2C5D3}PLERP.net 1.8: Witaj, %s! Zalogowa³eœ siê jako {FFFFFF}%s {B2C5D3}(ID: %d, UID: %d, GID: %d).", pInfo[playerid][gname], pInfo[playerid][name], playerid, pInfo[playerid][uid], pInfo[playerid][gid]);
	   ...
	   {
	     SendClientMessage(playerid, COLOR_RED, "Jeszcze niedawno utraci³eœ przytomnoœc i jesteœ os³abiony, lepiej udaj siê do szpitala.");
	   }
       ...
       if( strval(qsTime) > 0 && gettime() - strval(qsTime) < 60*10)
       {
         ...
         format( formattedMsg, 128, "PLERP.net: Twoja poprzednia pozycja zosta³a przywrócona, czas od wylogowania: %dm %ds", timeFromLogout/60, timeFromLogout%60 );
       ...
       else
       {
		 ...
		 if( spawnType == 2 ) 
		 {
			...
			SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby wyjœc z pokoju hotelowego u¿yj /pokoj wyjdz");
		 }
       }
	...
	else 
	{
	   ...
	   {
		  format( string, sizeof(string), "PLERP.net: Wpisa³eœ b³êdne has³o %d razy - zostaniesz wyrzucony z serwera!", MAX_LOGIN_ATTEMPTS); 
		  SendClientMessage(playerid, COLOR_GREY, string); 
		  Kick(playerid);
	   }
	   format( string, sizeof(string), "Witaj %s, pod³¹czy³eœ siê do serwera PLERP.net.\n\
	                                         Dziœ jest %s, %d dzieñ w roku.\n\n",globaln[playerid], FormatDataForShow(gettime(), 1), getdate(tmpInt, tmpInt, tmpInt));
	   format( string2, sizeof(string2), "Aby rozpocz¹æ grê postaci¹ {DCD6BC}%s {a9c4e4}zaloguj siê podaj¹c has³o\n\
										  w polu poni¿ej. Mo¿esz tak¿e skorzystaæ z dodatkowych opcji.\n\n\
										         \t{9f000b}Wpisane has³o jest b³êdne - pozosta³o %d prób.", pName[playerid], (MAX_LOGIN_ATTEMPTS-pLoginAttempt[playerid]) );
...
public OnPlayerSpawn(playerid)
{
    ...
    if( pInfo[playerid][logged] == 1 && pInfo[playerid][bw] > 0 )
	{
	   ...
	   SendClientMessage(playerid, COLOR_RED, "Zosta³eœ powa¿nie ranny i straci³eœ przytomnoœæ. Poczekaj na przybycie karetki, lub odzyskanie przytomnoœci.");
	}
...
public OnPlayerText(playerid, text[])
{
	if(pInfo[playerid][bw] > 0)
    {
        SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie mo¿esz mówiæ gdy jesteœ nieprzytomny.");
        return 0;
    }
	
	if( !strcmp(":)", text, true) ) { NewProx(playerid, "me", "uœmiecha siê"); return 0; }
	if( !strcmp(":(", text, true) ) { NewProx(playerid, "me", "smuci siê"); return 0; }
	if( !strcmp("xd", text, true) ) { NewProx(playerid, "me", "wybucha œmiechem"); return 0; }
	if( !strcmp(":D", text, true) ) { NewProx(playerid, "me", "œmieje siê"); return 0; }
	if( !strcmp(":/", text, true) ) { NewProx(playerid, "me", "krzywi siê"); return 0; }
	if( !strcmp(":\\", text, true) ) { NewProx(playerid, "me", "krzywi siê"); return 0; }
	...
	if( text[0] == 64 )
	{
	  ...
	  { SendClientMessage(playerid, COLOR_GREY, "PLERP.net: @[slot 1-5] [wiadomoœæ]"); return 0; }
	...
	if( pCall[playerid][cState] == 2 )
	{
	  ...
	  if( pCall[playerid][cReceiver] == -911 )
	  {
	  	if(pInfo[playerid][t_911] == 1)
        {
                ...
                {
                        SendClientMessage(playerid, COLOR_GREEN, "E911: Rozumiem, proszê okreœliæ rodzaj przestêpstwa.");
                        pInfo[playerid][t_911] = 0;
                        pInfo[playerid][t_911_police] = 1;
                } else if(!strcmp(text, "Pogotwie", true))
                {
                        SendClientMessage(playerid, COLOR_GREEN, "E911: Rozumiem, proszê okreœliæ rodzaj wypadku.");
                        pInfo[playerid][t_911] = 0;
        ...
        if(pInfo[playerid][t_911_ambulance] != 0)
        {
                switch(pInfo[playerid][t_911_ambulance])
                {
                        case 1:
                        {
                                ...
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Proszê okreœliœæ iloœæ ofiar. (( s³ownie ))");
                                pInfo[playerid][t_911_ambulance] = 2;
                        }
                        case 2:
                        {
                                format(pInfo[playerid][tmp2], 128, "%s", text);
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Proszê podaæ dok³adne miejsce zdarzenia.");
                                pInfo[playerid][t_911_ambulance] = 3;
                        }
                        case 3:
                        {
                                format(pInfo[playerid][tmp3], 256, "%s", text);
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Mo¿e Pan/Pani podaæ swoje dane?");
                                pInfo[playerid][t_911_ambulance] = 4;
                        }
                        case 4:
                        {
                                format(pInfo[playerid][tmp4], 128, "%s", text);
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Zg³oszenie zosta³o przyjête, odpowiednie jednostki s¹ ju¿ w drodze.");
        ...
        {      
                switch(pInfo[playerid][t_911_police])
                {
                        ...
                        {
                                format(pInfo[playerid][tmp1], 128, "%s", text);
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Proszê okreœliæ iloœæ sprawców. (( s³ownie ))");
                                pInfo[playerid][t_911_police] = 2;
                        }
                        case 2:
                        {
                                format(pInfo[playerid][tmp2], 128, "%s", text);
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Sprawcy s¹ znani? Jeœli tak to proszê ich opisaæ, podaæ ich dane personalne.");
                                pInfo[playerid][t_911_police] = 3;
                        }
                        case 3:
                        {
                                format(pInfo[playerid][tmp3], 256, "%s", text);
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Proszê dok³adnie okreœliæ miejsce incydentu.");
                        ...
                        case 5:
                        {
                                ...
                                SendClientMessage(playerid, COLOR_GREEN, "E911: Zg³oszenie przyjête, odpowiednie jednostki s¹ ju¿ w drodze.");
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if( sVehInfo[vehicleid][destroyed] && !ispassenger )
	{
      ...
      ShowPlayerDialog(playerid, 3, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Pojazd zniszczony"), "Twój pojazd jest ca³kowicie zniszczony.\n\
	                                                                                            Mog³o do tego dojœæ wskutek wybuchu lub wpadniêcia do wody.\n\n\
																								Aby przywróciæ go do stanu u¿ywalnoœci, musisz poprosiæ kogoœ\n\
																								o zaholowanie pojazdu do warsztatu, gdzie zajm¹ siê nim mechanicy.\n\n\
																								Aby zaakceptowaæ ofertê naprawy, bêdziesz musia³ siedzieæ w œrodku, jako pasa¿er.", "Zamknij", "");
	}
    else if( sVehInfo[vehicleid][locked])
	{
	  ClearAnimations(playerid);
	  ShowPlayerDialog(playerid, 3, DIALOG_STYLE_MSGBOX, BuildGuiCaption("B³¹d - Pojazdy"), "Pojazd jest zamkniêty", "Zamknij", "");
    }
...
{	
	...
	if(newkeys & 1)
	{
      ...
      if( sVehInfo[vid][carActionInProgress] ) return ShowPlayerDialog(playerid, 3, DIALOG_STYLE_MSGBOX, BuildGuiCaption("B³¹d - Pojazdy"), "Aktualnie nie mo¿esz odpaliæ silnika, poniewa¿ pojazd jest w akcji.", "Zamknij", "");
	  ...
	  { 
	    if( GetVehicleCurrentFuel(vid) == 0.0 ) return ShowPlayerDialog(playerid, 3, DIALOG_STYLE_MSGBOX, BuildGuiCaption("B³¹d - Pojazdy"), "Brak paliwa w pojeŸdzie! Pomyœl o zakupieniu kanistra lub zadzwoñ do warsztatu samochodowego.", "Zamknij", "");
     ...
     if(weaponid == 0)
        {
                foreach(Items_PLAYER[issuerid], i)
                {
                        if(Item[i][value2] == 666 && Item[i][used] == 1)
                        {
                                ...
                                format(string, sizeof(string), "Zosta³eœ pora¿ony paralizatorem przez %s.", pInfo[issuerid][name]);
                                SendClientMessage(playerid, COLOR_RED, string);
                                format(string, sizeof(string), "Porazi³eœ paraliztorem %s.", pInfo[playerid][name]);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch( dialogid )
	{
	  case 0:
	  {
		...
		{
		  new string[200];
	      format( string, sizeof(string), "Zmieñ postaæ\nWyjd¿ z serwera" );
		  ShowPlayerDialog(playerid, 71, DIALOG_STYLE_LIST, BuildGuiCaption("PLERP.net » Logowanie » Opcje"), string, "Wybierz", "Wróæ");
		}
	  }
	  ...
	  case 4:
	  {
		...
		{
		  ShowPlayerDialog(playerid, 43, DIALOG_STYLE_INPUT, BuildGuiCaption("Pojazd - Radio stacja"), "Aby zmieniæ nadawan¹ stacjê wklej poni¿ej link do jej streama, listê polskich streamów znajdziesz pod adresem:\n   {FFBC79} http://www.listenlive.eu/poland.html", "Zmieñ", "Zamknij");
		}
		...
		if( response == 1 )
		{
		  ...
		  if( listitem > 0  )
		  {
		    if( !strcmp(lastGroupGuiOptions[playerid][listitem-1], "special", true) )
		    {
			  ...
			  {
                format(string2, 128, "%s\n%s", string2, "Stwórz drzwi"); format(lastGroupGuiOptions[playerid][i], 34, "door-create"); i++;
                format(string2, 128, "%s\n%s", string2, "Stwórz pojazd"); format(lastGroupGuiOptions[playerid][i], 34, "vehicle-create"); i++;
                format(string2, 128, "%s\n%s", string2, "Stwórz przedmiot"); format(lastGroupGuiOptions[playerid][i], 34, "item-create"); i++;
	  ...
	  case 11:
	  {
		...
		{
		  if( listitem == 0 ) ShowPlayerDialog(playerid, 12, DIALOG_STYLE_INPUT, BuildGuiCaption("Tworzenie drzwi"), "** aby utworzyæ drzwi musisz podaæ ich parametry w nastêpuj¹cej kolejnoœæi(pamiêtaj o odzielaniu ich przecinkami):\n   {efe4b0}         [TYP_DRZWI],[UID_GRUPY/GRACZA],[TYP_IKONY_DRZWI],[STYL_IKONY_DRZWI]\n  {a9c4e4}Nie zapominaj, ¿e drzwi zostan¹ stworzone w miejscu, w którym aktualnie siê znadujesz!", "Stwórz", "Zamknij");
		  if( listitem == 1 ) ShowPlayerDialog(playerid, 39, DIALOG_STYLE_INPUT, BuildGuiCaption("Tworzenie pojazdu"), "** aby utworzyæ pojazd musisz podaæ jego parametry w nastêpuj¹cej kolejnoœæi(pamiêtaj o odzielaniu ich przecinkami):\n   {efe4b0}         [ID_MODELU],[ID_KOLORU_1],[ID_KOLORU_2],[POJEMNOSC_BAKU]\n  {a9c4e4}Nie zapominaj, ¿e pojazd zostanie stworzonu w miejscu, w którym aktualnie siê znadujesz!", "Stwórz", "Zamknij");
		  if( listitem == 3 ) {
		    new options[180];
		    new usedObjects = Iter_Count(Objects_GLOBAL);
			format(options, 180, "Dostêpne obiekty: %s (%d u¿ytych)\nTryb wyboru obiektów(L.ALT + Y)\nStwórz obiekt\nUsuñ obiekt", "NIEOGR.", usedObjects);
			ShowPlayerDialog(playerid, 36, DIALOG_STYLE_LIST, BuildGuiCaption("Zarz¹dzanie obiektami"), options, "Wybierz", "Zamknij");
		  }
		  if( listitem == 4 ) {
		    new options[180];
			format(options, 180, "Stwórz bramê\n------------------------\n");
			ShowPlayerDialog(playerid, 40, DIALOG_STYLE_LIST, BuildGuiCaption("Zarz¹dzanie bramami"), options, "Wybierz", "Zamknij");
		  }
		  if( listitem == 5 ) ShowPlayerDialog(playerid, 19, DIALOG_STYLE_LIST, BuildGuiCaption("Dodatki"), "Teleport w przód o 3.5 jednostki metrycznej\nTeleport w przód o x metrów na najwy¿szy punkt", "Wybierz", "Zamknij");
	  ...
	  case 12:
	  {
		...
		{
          ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Tworzenie drzwi"), "Dane, które poda³eœ podczas tworzenia drzwi s¹ niekompletne", "Zamknij", "");
		}
		else
		{
	  ...
	  case 13:
	  {
		if( response == 1 )
		{
		   ...
		   if( listitem == 0 )
		   {
              ShowPlayerDialog(playerid, 14, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "** aby nazwa by³a bardziej unikatowa mo¿esz u¿yæ kolorów:\n   {efe4b0}   ~g~, ~r~, ~b~, ~w~, ~y~, ~p~, ~l~, ~h~\n", "Zmieñ", "Zamknij");
		   }
		   else if( listitem == 1 )
		   {
		      if( doors[areaId][type ] == 1 ) ShowPlayerDialog(playerid, 15, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "Nie mo¿esz ustawiæ op³aty wejœciowej w prywatnych drzwiach.", "Zamknij", "");
              else ShowPlayerDialog(playerid, 15, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "** kwota, któr¹ ustawisz bêdzie pobierana od ka¿dego gracza w momencie przejœcia przez drzwi.", "Zmieñ", "Zamknij");
		   }
		   else if( (listitem == 2 && areaId != -1) )
		   {
              ...
              mysql_function_query(mysqlHandle, Query, false, "", "");
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Twoja aktualna pozycja zosta³a ustawiona jako pozycja wejœciowa drzwi!", "Zamknij", "");
		   }
	  ...
	  case 15:
	  {
	    ...
	    {
           ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Podana wartoœæ nie jest liczb¹.", "Zamknij", "");
	    }
	    else
	    {
           new Query[256];
           ...
           ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Kwota pobierana za przejœcie przez drzwi zosta³a zmieniona!", "Zamknij", "");
	    }
	  }
	  ...
	  case 16:
	  {
        ...
        format(string, 64, "Edycja drzwi - [UID: %d]", doors[areaId][doorUid]);
		if( listitem == 2 ) ShowPlayerDialog(playerid, 18, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "** Podaj id modelu obiektu, który chcesz stworzyæ", "Stwórz", "Zamknij");
		if( listitem == 3 ) ShowPlayerDialog(playerid, 20, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "** Podaj id obiektu, który ma byæ usuniêty.\n  (mo¿esz go sprawdziæ poprzez tryb wybierania obiektu)", "Usuñ", "Zamknij");
	  }
	  ...
	  case 18:
	  {
        ...
        if( doors[areaId][maxObjects]-usedObjects == 0 ) { ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Nie posiadasz dostêpnych obiektów do wykorzystania.", "Zamknij", ""); return 1; }
		if( sscanf(inputtext, "d", oModelId) ) { ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "B³êdne ID modelu obiektu.", "Zamknij", ""); return 1; }
	  ...
	  case 20:
	  {
        ...
        format(string, 64, "Edycja drzwi - [UID: %d]", doors[areaId][doorUid]);
		if( sscanf(inputtext, "d", objId) ) { ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Podana wartoœæ nie jest liczb¹.", "Zamknij", ""); return 1; }
		if( objects[objId][objectVW] != doors[areaId][doorUid] ) { ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Obiekt, który chcesz usunaæ nie nale¿y do tych drzwi.", "Zamknij", ""); return 1; }
      ...
      case 25:
      {
		...
		{
          ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Podana wartoœæ nie jest poprawnym id gracza.", "Zamknij", "");
		}
		else
		{
		  if( playerid == offerFor )
		  {
            ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Nie mo¿esz wyrzuciæ samego siebie.", "Zamknij", "");
            return 1;
		  }
		  ...
		  {
            ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Nie mo¿esz wyrzuciæ lidera grupy.", "Zamknij", "");
            return 1;
		  }
		  if( IsPlayerInGroup(offerFor, grIdx) == 1 )
          {
			...
			ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Wyrzuci³eœ z grupy gracza o podanym id.", "Zamknij", "");
		  }
		  else
		  {
      ...
      case 26:
      {
		...
		{
          ShowPlayerDialog(playerid, 28, DIALOG_STYLE_LIST, BuildGuiCaption(GUIcaption), "Napisz wiadomoœæ\nPrzegl¹daj wiadomoœci", "Wybierz", "Zamknij");
		}
		else if( listitem == 2 )
		{
          ShowPlayerDialog(playerid, 29, DIALOG_STYLE_LIST, BuildGuiCaption(GUIcaption), "Tryb:\t\tCichy (wybierz aby zmieniæ)", "Wybierz", "Zamknij");
		}
      }
      ...
      case 27:
      {
		...
		if( listitem == 0 )
		{
		  ShowPlayerDialog(playerid, 30, DIALOG_STYLE_INPUT, BuildGuiCaption("Telefon » Wybieranie numeru"), "Wpisz numer telefonu rozmówcy.", "Zadzwoñ", "Zamknij");
		}
		else if( listitem == 2 )
		{
      ...
      case 28:
      {
		...
		if( listitem == 0 )
		{
		  ShowPlayerDialog(playerid, 115, DIALOG_STYLE_INPUT, BuildGuiCaption("Telefon » Wysy³anie wiadomoœci [1/2]"), "Wpisz numer telefonu na których chcesz wys³aæ wiadomoœ:", "Dalej", "Zamknij");
		}
		else if( listitem == 1 )
		{
      ...
      case 30:
      {
		...
		else
		{
		  ...
		  {
            ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("B³¹d - Przedmioty"), "Podana wartoœæ nie jest liczb¹.", "Zamknij", "");
		  }
		  else
		  {
		    if( numberTo == 911 )
		...
		else
		{
		  if( listitem == 0 )
		  {
			...
			if( pNr-1 == 0 ) 
			{
			  ...
			  return ShowPlayerDialog(playerid, 111, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Telefon » Wysy³anie vCarda"), "Brak graczy w pobli¿u do których móg³byœ wys³aæ vCard.", "Zamknij", "");
			}
      ...
      /*case 34:
      {
		...
		else
		{
		  if( IsPlayerConnected(showTo) && pInfo[showTo][logged] == 1 )
		  {
			...
			{
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Poka¿ dowód"), "Wskazany przez Ciebie gracz jest zbyt daleko.", "Zamknij", "");
              return 1;
			}
		  }
		  else
		  {
            ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Poka¿ dowód"), "Wskazany przez Ciebie gracz nie jest zalogowany.", "Zamknij", "");
            return 1;
		  }
		}
	    ...
	    new guiCaption[128];
	    format(guiCaption, 128, "Dowód osobisty - %s", dowodWho);
      ...
      case 35:
      {
		...
		if( listitem == 0 )
		{
		  ...
		  format( basicCommands, 320, "%s{FFB871}Postaæ:\t\t{FFFFFF} /postac /qs /opis\t\t\t\t\n", basicCommands );
		...
		{
		  // -- Spis animacji -- //
		  SendClientMessage(playerid, COLOR_GREY, "{999A9C}PLERP.net: -[nazwa animacji], np. -idz2 #Lista dostêpnych animacji dostêpna jest poprzez /pomoc");
      ...
      case 36:
      {
		...
		new string[64];
		if( listitem == 1 ) SelectObject(playerid);
        format(string, 64, "Zarz¹dzanie obiektami œwiata");
		if( listitem == 2 ) ShowPlayerDialog(playerid, 37, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "** Podaj id modelu obiektu, który chcesz stworzyæ", "Stwórz", "Zamknij");
		if( listitem == 3 ) ShowPlayerDialog(playerid, 38, DIALOG_STYLE_INPUT, BuildGuiCaption(string), "** Podaj id obiektu, który ma byæ usuniêty.\n  (mo¿esz go sprawdziæ poprzez tryb wybierania obiektu)", "Usuñ", "Zamknij");
      }
      ...
      case 37:
	  {
        ...
        format(string, 64, "Zarz¹dzanie obietkami œwiata - Tworzenie");
	  ...
	  case 38:
	  {
        ...
        format(string, 64, "Zarz¹dzanie obiektami œwiata - Usuwanie");
		if( sscanf(inputtext, "d", objId) ) { ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Podana wartoœæ nie jest liczb¹.", "Zamknij", ""); return 1; }
		if( objects[objId][objectVW] != 0 ) { ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption(string), "Obiekt, który chcesz usunaæ nie nale¿y do obiektów œwiata.", "Zamknij", ""); return 1; }
        Object_Remove(objId);
	  }
	  ...
	  case 39:
	  {
		...
		{
          ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Tworzenie pojazdu"), "Dane, które poda³eœ podczas tworzenia pojazdu s¹ niekompletne", "Zamknij", "");
		}
		else
		{
	  ...
	  case 40:
	  {
        ...
        if( listitem == 0 && strlen(currentDesc) > 0 )
		{
          ...
          SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Twój aktualny opis zosta³ usuniêty.");
		}
		else if( listitem == 0 && strlen(currentDesc) == 0 )
		{
          ShowPlayerDialog(playerid, 41, DIALOG_STYLE_INPUT, BuildGuiCaption("Postac - Opis"), "Poni¿ej wpisz opis, który chcesz ustawiæ. (max. 100 znaków)", "Ustaw", "Zamknij");
		}
		else if( listitem >= 2 )
		{
		  // -- Zmiana opisu na wczeœniej zapisany -- //
		  new Query[256];
	...
	case 46:// Opcje do przyczepialnego obiektu
	{
		...
		{	
			...
			if(freeslot == -1)
			{
				SendClientMessage(playerid, -1, "PLERP.net: Limit przyczepionych obiektów zosta³ przez Ciebie przekroczony.");
				return 1;
			}
	...
	case 47://Wysy³anie oferty przedmiotu
	{
	  ...
	  {
	     SendClientMessage(playerid, -1, "PLERP.net: Oferta nie zosta³a wys³ana, poniewa¿ poda³eœ b³êdne dane.");
	  }
	  else
	  {
	     if( !IsPlayerConnected(offerTo) || !pInfo[offerTo][logged] ) return SendClientMessage(playerid, -1, "PLERP.net: Oferta nie zosta³a wys³ana poniewa¿ gracz, którego poda³eœ nie jest zalogowany.");
	     
		 new Float:posX, Float:posY, Float:posZ;
		 GetPlayerPos(playerid, posX, posY, posZ); 
		 if( !IsPlayerInRangeOfPoint(offerTo, 5.0, posX, posY, posZ)  ) return SendClientMessage(playerid, -1, "PLERP.net: Oferta nie zosta³a wys³ana poniewa¿ gracz, którego poda³eœ jest zbyt daleko.");
	...
	case 50:
	{
		...
		if(!listitem)
		{
			ShowPlayerDialog(playerid, 51, DIALOG_STYLE_INPUT, "Metryki Zdrowotne -> Szukaj", "Poni¿ej podaj numer ubezpieczenia((UID GRACZA)) pacjenta b¹dŸ jego nazwisko", "Wyszukaj", "WyjdŸ");
		
		}
		else
		{
			ShowPlayerDialog(playerid, 52, DIALOG_STYLE_INPUT, "Metryki Zdrowotne -> Dodaj", "Poni¿ej podaj dane pacjenta w formacie\n Numer Ubezpieczenia((UID GRACZA));Imie;Nazwisko;Rok Urodzenia;Wzrost;Waga", "Dodaj", "WyjdŸ");
    ...
    case 49: // -- /kup
	{
	   ...
	   {
	     SendClientMessage(playerid, -1, "PLERP.net: Nie masz wystarczaj¹cej iloœci pieniêdzy, aby zakupiæ wybrany przedmiot.");
		 return 1;
	   }
	   ...
	   format( string, sizeof(string), "Kupi³eœ przedmiot %s za $%d. Dziêkujemy za zakup!", Item[itemIDX][name], Item[itemIDX][price] );
	...
	case 70: // Tworzenie stref
	{
	  ...
	  {
         SendClientMessage(playerid, -1, "PLERP.net: Poda³eœ b³êdne dane.");
         return 1;		 
	  }
	  else
	  {
	...
	case 71:
	{
	  ...
	  {
	    new string[100];
		format( string, sizeof(string), "Wpisz {DCD6BC}Imie_Naziwsko {a9c4e4}postaci, na któr¹ chcesz siê zalogowaæ." );
		ShowPlayerDialog(playerid, 72, DIALOG_STYLE_INPUT, BuildGuiCaption("PLERP.net » Logowanie » Zmieñ postaæ"), string, "Wybierz", "Wróæ");
	  }
	}
	...
	case 74:
	{
	  if(!response) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Przepisywanie pojazdu pod grupê zosta³o anulowane.");
	  ...
	  else return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Przepisywanie pojazdu pod grupê zosta³o anulowane.");
	}
	...
	case 54:
	{
		...
		switch(listitem)
		{
			...
			case 1:
			ShowPlayerDialog(playerid, 55, DIALOG_STYLE_INPUT, "Metryka Zdrowotna -> Drukowanie", "Poni¿ej podaj dodatkowy komentarz, który ma byæ wydrukowany.", "Ok", "WyjdŸ");
			case 2:	
	...
	case 56:
	{
		...
		if(!listitem)
		{
			ShowPlayerDialog(playerid, 58, DIALOG_STYLE_INPUT, "Metryki Zdrowotne -> Dodaj Wpis", "Poni¿ej podaj treœæ jak¹ ma zawieraæ wpis", "Wybierz", "WyjdŸ");
		}
		else
		{
	...
	case 57:
	{
		...
		if(!listitem)
		{
			...
			mysql_function_query(mysqlHandle, zapytanie, false, "", "");
			//ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Metryki Zdrowotne -> Informacja", "Wpis zosta³ pomyœlnie usuniêty", "OK", "");
			format(zapytanie, sizeof(zapytanie), "SELECT * FROM `plerp_metryczki_more` WHERE `owner` = %i", pInfo[playerid][t_dialogtmp1]);
			mysql_function_query(mysqlHandle, zapytanie, true, "MetryczkiWpisyMysqlQuery", "i", playerid);
		}
	}
	case 58:
	{
		...
		mysql_function_query(mysqlHandle, zapytanie, false, "", "");
		ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Metryki Zdrowotne -> Informacja", "Wpis zosta³ pomyœlnie dodany", "OK", "");
		format(zapytanie, sizeof(zapytanie), "SELECT * FROM `plerp_metryczki_more` WHERE `owner` = %i", pInfo[playerid][t_dialogtmp1]);
		mysql_function_query(mysqlHandle, zapytanie, true, "MetryczkiWpisyMysqlQuery", "i", playerid);
	}
	case 59:
	...
	case 61:
	{
		...
		mysql_function_query(mysqlHandle, zapytanie, false, "", "");
		ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "P³yta", "P³yta zosta³a umieszczona pomyœlnie w discmanie", "OK", "");
	...
	case 62:
	{
		...
		else//Wyci¹gniecie p³yty
		{
			...
			mysql_function_query(mysqlHandle, zapytanie, false, "", "");
			ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Wyjêcie p³yty", "Pomyœlnie wyj¹³eœ p³ytê z discman'a", "OK", "");
		}
	}
	case 63:
	{
		...
		if(Item[ItemIDX][owner_type] != OWNER_TYPE_WAREHOUSE) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Przedmiot który chcesz oferowaæ nie istnieje", "Zamknij", "");
		//Jestes chujem 2
	  ...
	  case 80: // COMMAND:db
		{
			...
			switch(listitem)
			{
				...
				case 1: // Dodaj wpis
				{
					ShowPlayerDialog(playerid, 84, DIALOG_STYLE_LIST, "Baza danych LSPD -> Dodaj wpis", "Dodaj poszukiwany pojazd\nDodaj poszukiwan¹ osobê\nDodaj zarekwirowany pojazd\nDodaj kartotekê", "Wybierz", "Anuluj");
				}
				case 2: // Usuñ wpis
				{
					if(!HasPlayerPermission(playerid, "group", GPREM_leader, 4)) return ShowPlayerDialog(playerid, 100, DIALOG_STYLE_MSGBOX, "Baza danych LSPD -> Usuñ wpis", "Nie jesteœ liderem Los Santos Police Department.", "OK", "");
					ShowPlayerDialog(playerid, 86, DIALOG_STYLE_LIST, "Baza danych LSPD -> Usuñ wpis", "Usuñ poszukiwany pojazd\nUsuñ poszukiwan¹ osobê\nUsuñ zarekwirowany pojazd\nUsuñ kartotekê", "Wybierz", "Anuluj");
				}
				case 3: // Edytuj wpis
				{
					ShowPlayerDialog(playerid, 88, DIALOG_STYLE_LIST, "Baza danych LSPD -> Edytuj wpis", "Edytuj poszukiwany pojazd\nEdytuj poszukiwan¹ osobê\nEdytuj zarekwirowany pojazd\nEdytuj kartotekê", "Wybierz", "Anuluj");
				}
			}
		}
		case 81: // Baza LSPD -> Szukaj
		{
			...
			switch(listitem)
			{
				case 0:
				{
					ShowPlayerDialog(playerid, 82, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Szukaj", "Wpisz rejestracjê poszukiwanego pojazdu.", "Akceptuj", "Anuluj");
				...
				case 2:
				{
					ShowPlayerDialog(playerid, 82, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Szukaj", "Wpisz markê odholowanego pojazdu.", "Akceptuj", "Anuluj");
				...
				case 4: 
				{
					ShowPlayerDialog(playerid, 82, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Szukaj", "Wpisz imiê i nazwisko na które zosta³ wypisany mandat w formacie: \
																		\nImiê;Nazwisko", "Akceptuj", "Anuluj");
					pInfo[playerid][t_dialogtmp1] = 5; // Mandaty
				}
				case 5:
				{
					ShowPlayerDialog(playerid, 82, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Szukaj", "Wpisz rejestracjê pojazdu na który zosta³a na³o¿ona blokada.", "Akceptuj", "Anuluj");
					pInfo[playerid][t_dialogtmp1] = 6; // Bloady kó³
				}
			}
		}
		...
		case 84:
		{
			...
			switch(listitem)
			{
				...
				case 3: // Dodaj kartotekê
				{
					ShowPlayerDialog(playerid, 85, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Dodaj wpis", "Dodaj wpis w formacie: \
																		\nImiê;Nazwisko;Ksywa;Adres;Pojazdy;Wzrost;Kolor skóry;Kolor oczu;Aresztowania(iloœæ);Powody aresztowañ", "Akceptuj", "Anuluj");
			...
			switch(int)
			{
				case 1: // Baza danych LSPD -> Dodaj wpis -> Poszukiwany pojazd
				{
					...
					} else
					{
						...
						ShowPlayerDialog(playerid, 100, DIALOG_STYLE_MSGBOX, "Baza danych LSPD -> Dodaj wpis", "Utworzy³eœ nowy wpis w bazie danych LSPD", "OK", "");
					}
				}
		...
		case 86:
		{
			...
			switch(listitem)
			{
				case 0: // Baza danych LSPD -> Usuñ wpis -> Poszukiwany pojazd
				{
					ShowPlayerDialog(playerid, 87, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Usuñ wpis", "Wpisz numer (UID) wpisu który chcesz usun¹æ", "Usuñ", "WyjdŸ");
					pInfo[playerid][t_dialogtmp1] = 1;
				}
				case 1: // Baza danych LSPD -> Usuñ wpis -> Poszukiwana osoba
				{
					ShowPlayerDialog(playerid, 87, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Usuñ wpis", "Wpisz numer (UID) wpisu który chcesz usun¹æ", "Usuñ", "WyjdŸ");
					pInfo[playerid][t_dialogtmp1] = 2;
				}
				case 2: // Baza danych LSPD -> Usuñ wpis -> Zarekwirowany pojazd
				{
					ShowPlayerDialog(playerid, 87, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Usuñ wpis", "Wpisz numer (UID) wpisu który chcesz usun¹æ", "Usuñ", "WyjdŸ");
					pInfo[playerid][t_dialogtmp1] = 3;
				}
				case 3: // Baza danych LSPD -> Usuñ wpis -> Kartoteka
				{
					ShowPlayerDialog(playerid, 87, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Usuñ wpis", "Wpisz numer (UID) wpisu który chcesz usun¹æ", "Usuñ", "WyjdŸ");
					pInfo[playerid][t_dialogtmp1] = 4;
				}
		...
		case 87:
		{
			if(!response) return ShowPlayerDialog(playerid, 86, DIALOG_STYLE_LIST, "Baza danych LSPD -> Usuñ wpis", "Usuñ poszukiwany pojazd\nUsuñ poszukiwan¹ osobê\nUsuñ zarekwirowany pojazd\nUsuñ kartotekê", "Wybierz", "Anuluj");
			...
			switch(int)
			{
				case 1: // Baza danych LSPD -> Usuñ wpis -> Poszukiwany pojazd
				{	
				...
				case 2: // Baza danych LSPD -> Usuñ wpis -> Poszukiwana osoba
				{
				...
				case 3: // Baza danych LSPD -> Usuñ wpis -> Zarekwirowany pojazd
				{
				...
				case 4: // Baza danych LSPD -> Usuñ wpis -> Kartoteka
				{
		...
		case 88:
		{
			...
			switch(listitem)
			{
				...
				{
					ShowPlayerDialog(playerid, 89, DIALOG_STYLE_INPUT, "Baza danych LSPD -> Edytuj wpis", "Wpisz numer (UID) wpisu który chcesz edytowaæ.", "Edytuj", "Anuluj");
					pInfo[playerid][t_dialogtmp1] = 1;
				}
			...
			switch(int)
			{
				case 1: // Baza danych LSPD -> Edytuj wpis -> Poszukiwany pojazd
				{
					...
					} else
					{
						...
						ShowPlayerDialog(playerid, 100, DIALOG_STYLE_MSGBOX, "Baza danych LSPD -> Edytuj", "Z powodzeniem zmieni³eœ wpis w bazie danych LSPD.", "OK", "");
					}
				}
				case 2: // Baza danych LSPD -> Edytuj wpis -> Poszukiwana osoba
				{
					...
					} else
					{
						...
						ShowPlayerDialog(playerid, 100, DIALOG_STYLE_MSGBOX, "Baza danych LSPD -> Dodaj wpis", "Z powodzeniem zmieni³eœ wpis w bazie danych LSPD", "OK", "");
					}
				}
				case 3: // Baza danych LSPD -> Edytuj wpis -> Zarekwirowany pojazd
				{
				...
				case 4: // Baza danych LSPD -> Edytuj wpis -> Kartoteka
				{
					...
					} else
					{
						...
						ShowPlayerDialog(playerid, 100, DIALOG_STYLE_MSGBOX, "Baza danych LSPD -> Edytuj wpis", "Z powodzeniem zmieni³eœ wpis w bazie danych LSPD.", "OK", "");
					}
				}
			}
		}	
		case 91: // COMMAND: mandat -> Wypisz mandat
		{
			...
			} else
			{
				...
				format(string, sizeof(string), "Wypisa³eœ mandat w wysokoœci $%d oraz %d punktów karnych graczowi %s. Powód: %s.", money, pkt, pInfo[id][name], reason);
				SendClientMessage(playerid, COLOR_GREEN, string);
				format(string, sizeof(string), "Funkcjonariusz %s wypisa³ Ci mandat w wysokoœci $%d oraz %d punktów karnych. Powód: %s.", pInfo[id][name], money, pkt, reason);
		...
		case 92: // COMMAND: mandat -> Zdejmij mandat
		{
			...
			ShowPlayerDialog(playerid, 100, DIALOG_STYLE_MSGBOX, "Zdejmij mandat", "Z powodzeniem zdj¹³eœ mandat.", "OK", "");
		}
		case 93: // COMMAND: blokada -> naloz
		{
			...
			} else
			{
				...
				format(string, sizeof(string), "Za³o¿y³eœ blokadê na ko³o pojazdowi o rejestracji: %s", reg);
				SendClientMessage(playerid, COLOR_RED, string);
			}
		}
		...
		case 95: // COMMAND: /usmierc 1/2
        {
            ...
            ShowPlayerDialog(playerid, 96, DIALOG_STYLE_INPUT, "Uœmieræ postaæ", "Wpisz okolicznoœci œmierci swojej postaci.", "Akceptuj", "Anuluj");
        }
		...
		case 98: // COMMAND:/ban 1/2 AND COMMAND:/aj 1/2
                                ...
                                case 1:
                                {
                                        switch(listitem)
                                        {
                                                case 0: // 24h
                                                {
                                                        ...
                                                        ShowPlayerDialog(playerid, 99, DIALOG_STYLE_INPUT, "Powód", "Wpisz powód nadania banicji", "Zbanuj", "Anuluj");
                                                }
                                                case 1: // 48h
                                                {
                                ...
                                case 2:
                                {
                                        switch(listitem)
                                        {
                                                case 0: // 30 min
                                                {
                                                        ...
                                                        ShowPlayerDialog(playerid, 99, DIALOG_STYLE_INPUT, "Powód", "Wpisz powód nadania Admin Jaila", "Akceptuj", "Anuluj");
                                                }
                                                case 1: // 1h
                                                {
                ...
                {
                        if(!response) return ShowPlayerDialog(playerid, 102, DIALOG_STYLE_LIST, "Admin Control Panel", "Podstawowe komendy\nBlokady\nDodatki", "Wybierz", "Wróæ");
                       
                        switch(listitem)
                        {
                                case 0: // COMMAND:acp -> Zbanuj gracza
                                {
                                        ShowPlayerDialog(playerid, 104, DIALOG_STYLE_INPUT, "Zbanuj gracza", "Wpisz ID gracza którego chcesz zbanowaæ.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 1;
                                }
                                case 1: // COMMAND:acp -> Wyrzuæ gracza
                                {
                                        ShowPlayerDialog(playerid, 104, DIALOG_STYLE_INPUT, "Wyrzuæ gracza", "Wpisz ID którego chcesz wyrzuciæ.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 2;
                                }
                                case 2: // COMMAND:acp -> Nadaj warna
                                {
                                        ShowPlayerDialog(playerid, 104, DIALOG_STYLE_INPUT, "Nadaj warna graczowi", "Wpisz ID gracza któremu chcesz nadaæ warna.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 3;
                                }
                                case 3: // COMMAND:acp -> Admin Jail
                                {
                                        ShowPlayerDialog(playerid, 104, DIALOG_STYLE_INPUT, "Wrzuæ gracza do Admin Jaila", "Wpisz ID gracza którego chcesz zamkn¹æ w Admin Jail.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 4;
                                }
                        }
                }
                case 104: // COMMAND:acp -> Podstawowe komendy
                {
                        ...
                        switch(pInfo[playerid][t_dialogtmp1])
                        {
                                ...
                                case 2: // Kick
                                {
                                        mysql_real_escape_string(inputtext, inputtext);
                                        pInfo[playerid][t_dialogtmp2] = strval(inputtext);
                                        ShowPlayerDialog(playerid, 100, DIALOG_STYLE_INPUT, "Wyrzuæ gracza", "Wpisz powód wyrzucenia gracza z serwera.", "Wyrzuæ", "Anuluj");
                                }
                                case 3: // Warn
                                {
                                        mysql_real_escape_string(inputtext, inputtext);
                                        pInfo[playerid][t_dialogtmp2] = strval(inputtext);
                                        ShowPlayerDialog(playerid, 101, DIALOG_STYLE_INPUT, "Nadaj warna graczowi", "Wpisz powód nadania warna graczowi.", "Akceptuj", "Anuluj");
                                }
                                case 4: // AJ
                                {
                                        mysql_real_escape_string(inputtext, inputtext);
                ...
                case 105: // COMMAND:acp -> Blokady
                {
                        ...
                        switch(listitem)
                        {
                                ...
                                {
                                        ShowPlayerDialog(playerid, 106, DIALOG_STYLE_INPUT, "Blokada czatu OOC", "Wpisz ID gracza któremu chcesz nadaæ blokadê.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 1;
                                }
                                case 1: // Blokada biegania
                                {
                                        ShowPlayerDialog(playerid, 106, DIALOG_STYLE_INPUT, "Blokada biegania", "Wpisz ID gracza któremu chcesz nadaæ blokadê.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 2;
                                }
                                case 2: // Blokada bicia
                                {
                                        ShowPlayerDialog(playerid, 106, DIALOG_STYLE_INPUT, "Blokada bicia", "Wpisz ID gracza któremu chcesz nadaæ blokadê.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 3;
                                }
                        }
                }
                case 106: // COMMAND:acp -> Blokady
                {
                        ...
                        switch(pInfo[playerid][t_dialogtmp1])
                        {
                                case 1: // Blokada czatu OOC
                                {
                                        mysql_real_escape_string(inputtext, inputtext);
                                        pInfo[playerid][t_dialogtmp2] = strval(inputtext);
                                        ShowPlayerDialog(playerid, 107, DIALOG_STYLE_INPUT, "Blokada czatu OOC", "Wpisz powód nadania blokady.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 1;
                                }
                                case 2: // Blokada biegania
                                {
                                        mysql_real_escape_string(inputtext, inputtext);
                                        pInfo[playerid][t_dialogtmp2] = strval(inputtext);
                                        ShowPlayerDialog(playerid, 107, DIALOG_STYLE_INPUT, "Blokada biegania", "Wpisz powód nadania blokady.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 2;
                                }
                                case 3: // Blokada bicia
                                {
                                        mysql_real_escape_string(inputtext, inputtext);
                                        pInfo[playerid][t_dialogtmp2] = strval(inputtext);
                                        ShowPlayerDialog(playerid, 107, DIALOG_STYLE_INPUT, "Blokada bicia", "Wpisz powód nadania blokady.", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 3;
                                }
                        }
                }
                ...
                case 109:
                {
                        ...
                        switch(listitem)
                        {
                                case 0: // Teleportacja
                                {
                                        ShowPlayerDialog(playerid, 110, DIALOG_STYLE_INPUT, "Teleportuj", "Wpisz ID graczy których chcesz teleportowaæ w formacie:\n\
                                                                                                                                                                                ID teleportowanego;ID gracza docelowego", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 1;
                                }
                                case 1: // Slapowanie
                                {
                                        ShowPlayerDialog(playerid, 110, DIALOG_STYLE_INPUT, "Slapuj", "Wpisz ID gracza którego chcesz zeslapowaæ.", "Slap", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 2;
                                }
                                case 2: // Nadaj HP
                                {
                                        ShowPlayerDialog(playerid, 110, DIALOG_STYLE_INPUT, "Nadaj HP", "Wpisz dane w formacie:\n\
                                                                                                                                                                                ID gracza;Iloœæ ¿ycia", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 3;
                                }
                                case 3: // Nadaj pancerz
                                {
                                        ShowPlayerDialog(playerid, 110, DIALOG_STYLE_INPUT, "Nadaj pancerz", "Wpisz dane w formacie:\n\
                                                                                                                                                                                        ID gracza;Iloœæ pancerza", "Akceptuj", "Anuluj");
                                        pInfo[playerid][t_dialogtmp1] = 4;
                                }
                                case 4: // Spec
                                {
                                        ShowPlayerDialog(playerid, 110, DIALOG_STYLE_INPUT, "Spectating", "Wpisz ID gracza którego chcesz podgl¹daæ.", "Akceptuj", "Anuluj");
		...
		case 112:
		{
		   ...
		   {
		     new msgTo[50];
			 format( msgTo, sizeof(msgTo), "Telefon » Wysy³anie wiadomoœci do %d [2/2]", pInfo[playerid][t_dialogtmp3] );
		     ShowPlayerDialog(playerid, 116, DIALOG_STYLE_INPUT, BuildGuiCaption(msgTo), "Aby wys³aæ wiadomoœc tekstow¹ na podany numer musisz podaæ jej treœæ w poni¿szym polu:", "Wyœlij", "Anuluj");
		   }
		   
		   if( listitem == 4 )
		   {
		     ShowPlayerDialog(playerid, 113, DIALOG_STYLE_INPUT, BuildGuiCaption("Telefon » Edycja kontaktu » Zmieñ nazwê"), "Podaj now¹ nazwê kontaktu. Nazwa ta bêdzie równie¿ wyœwietlana w spisie po³¹czeñ, wiadomoœciach oraz na ekranie po³¹czenia.", "Zapisz", "Anuluj");
		   }
		   
		   if( listitem == 5 )
		   {
		     ShowPlayerDialog(playerid, 114, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Telefon » Edycja kontaktu » Usuñ"), "Czy na pewno chcesz usun¹æ ten kontakt? Ta operacja jest permanentna.", "Usuñ", "Anuluj");
		   }
		}
		...
		case 115:
		{
		  new GUIcaption[64], phoneIDX = pInfo[playerid][lastUsedPhone];
		  format(GUIcaption, 64, "%s » Menu", Item[phoneIDX][name]);
		  if(!response || isnull(inputtext) || !IsNumeric(inputtext) || strlen(inputtext) != 7 || strval(inputtext) == Item[phoneIDX][value1]) return ShowPlayerDialog(playerid, 28, DIALOG_STYLE_LIST, BuildGuiCaption(GUIcaption), "Napisz wiadomoœæ\nPrzegl¹daj wiadomoœci", "Wybierz", "Zamknij"); 
		...
		case 117:
		{
		  ...
		  if(!response) return ShowPlayerDialog(playerid, 28, DIALOG_STYLE_LIST, BuildGuiCaption(GUIcaption), "Napisz wiadomoœæ\nPrzegl¹daj wiadomoœci", "Wybierz", "Zamknij");
		...
		case 119: // LSN
        {
            switch(pInfo[playerid][t_dialogtmp1])
            {
                ...
                case 6: // Wywiad
                {
                    ...
                    {
                        SendPlayerOffer(targetid, playerid, OFFER_TYPE_LIVE, 0, 0);
                    } else return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Jeden z uczestników wywiadu nie znajduje siê w siedzibie LSN.");
                }
            }
        }
		...
		case 121: //Bank
		{
		  ...
		  format(string1, sizeof(string1), "Twoje konto bankowe zosta³o pomyœlnie za³o¿one!\n\n\
		                                    Numer konta: %d\n\
											Od teraz mo¿esz dokonywaæ wszelkich transakacji w banku\n\
											oraz bankomatach, które mo¿esz znaleœæ na terenie miasta.", bankNumber);
		...
		case 123: //Bank
		{
		  ...
		  {
		    new string1[150];
			format(string1, sizeof(string1), "Pamiêtaj, ¿e wszelkie wyp³aty jakie dostaniesz pojawi¹ siê w³aœnie tu - na Twoim koncie bankowym.\n\
			                                  {DCD6BC}Aktualne saldo: $%d", pInfo[playerid][bankcash]);
		    ShowPlayerDialog(playerid, 124, DIALOG_STYLE_MSGBOX, "Bank » Saldo konta", string1, "Wróæ", "");
		  }
		  else if( listitem == 3 )
		  {
		    ShowPlayerDialog(playerid, 125, DIALOG_STYLE_INPUT, "Bank » Wyp³ata", "Wpisz poni¿ej kwote, któr¹ chcesz wyp³aciæ ze swojego konta bankowego:", "Dalej", "Anuluj");
		  }
		  else if( listitem == 4 )
		  {
		    ShowPlayerDialog(playerid, 126, DIALOG_STYLE_INPUT, "Bank » Wp³ata", "Wpisz poni¿ej kwote, któr¹ chcesz wp³aciæ na swoje konto bankowe:", "Dalej", "Anuluj");
		  }
		  else if( listitem == 5 )
		  {
		    ShowPlayerDialog(playerid, 127, DIALOG_STYLE_INPUT, "Bank » Przelew [1/3]", "Wpisz poni¿ej numer konta, na który chcesz przelaæ pieni¹dze:", "Dalej", "Anuluj"); 
		  }
		}
		...
		case 125: //Bank
		{
		  ...
		  else
		  {
			...
			{
		      ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Wyp³ata", "Na swoim koncie bankowym nie posiadasz takiej kwoty pieniêdzy.", "Zamknij", "");		  
			}
			else
			{
			  AddBankMoney(pInfo[playerid][bank], -tCash);
			  AddPlayerMoney(playerid, tCash);
			  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Wyp³ata", "Wyp³aci³eœ ¿¹dan¹ kwotê pieniêdzy ze swojego konta bankowego.", "Zamknij", "");	
			}
		  }
		}
		...
		case 126: //Bank
		{
		  ...
		  else
		  {
			...
			{
			  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Wp³ata", "Przy sobie nie posiadasz takiej kwoty pieniêdzy.", "Zamknij", "");	
			}
			else
			{
			  AddBankMoney(pInfo[playerid][bank], tCash);
			  AddPlayerMoney(playerid, -tCash);
			  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Wp³ata", "Wp³aci³eœ ¿¹dan¹ kwotê pieniêdzy na swoje konto bankowe.", "Zamknij", "");	
			}
		  }
		}
		...
		case 128: //Bank
		{
		  ...
		  else
		  {
			...
			if( pInfo[playerid][cash] < tCash )
			{
			  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Przelew", "Na swoim koncie bankowym nie posiadasz takiej kwoty pieniêdzy.", "Zamknij", "");	
			}
			else
			{
	          ...
	          format(string1, sizeof(string1), "{f90012}Ta akcja wymaga potwierdzenia, poniewa¿ jest nieodwracalna.\n\n\
			                                    {a9c4e4}Czy na pewno chcesz przelaæ $%d na konto %d?\n\
												Aby potwierdziæ t¹ akcjê, wpisz poni¿ej 'potwierdzam':", pInfo[playerid][t_dialogtmp2], pInfo[playerid][t_dialogtmp1]);
		...
		case 129: //Bank
		{
		  ...
		  else
		  {
		    if( !strcmp(inputtext, "potwierdzam", true) )
			{
			  ...
			  {
			    return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Przelew", "Transakcja zosta³a anulowana, poniewa¿ na swoim koncie nie masz wymaganej iloœci pieniêdzy.", "Zamknij", "");	
			  }
			  ...
			  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Przelew", "Transakcja zosta³a wykonana, a pieni¹dzê powinny znajdowaæ siê na docelowym koncie.", "Zamknij", "");	
			}
			else
			{
			  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Bank » Przelew", "Transakcja zosta³a anulowana, poniewa¿ jej nie potwierdzi³eœ.", "Zamknij", "");	
			}
		  }
		}
		...
		{	
			...
			pInfo[playerid][t_dialogtmp1] = strval(inputtext);
			ShowPlayerDialog(playerid, 142, DIALOG_STYLE_INPUT, "Magazyn -> Step 1", "Podaj cene za któr¹ zamierzasz sprzedawaæ przedmiot.\nPamiêtaj, ¿e zawsze mo¿esz j¹ zmieniæ.", "Ok", "Zamknij");
		...
		case 142:
		{
			...
			if(pInfo[playerid][t_dialogtmp3] < 1 || pInfo[playerid][t_dialogtmp3] > 2000000) return ShowPlayerDialog(playerid, 141, DIALOG_STYLE_INPUT, "Magazyn -> Step 1", "Kwota musi byæ wiêksza od 0 i nie wiêksza od 2 000 000.\nPodaj cene za któr¹ zamierzasz sprzedawaæ przedmiot.\nPamiêtaj, ¿e zawsze mo¿esz j¹ zmieniæ.", "Ok", "Zamknij");
			ShowPlayerDialog(playerid, 143, DIALOG_STYLE_INPUT, "Magazyn -> Step 2", "Podaj iloœæ produktów któr¹ zamierasz zakupiæ.", "Ok", "Zamknij");
		}
		case 143:
		{
			if(!response) return ShowPlayerDialog(playerid, 140, DIALOG_STYLE_LIST, "Magazyn", "Gastronomia\nWarsztat\nGangi\nMafia\nPolicja", "Wybierz", "WyjdŸ");
			if(strval(inputtext) < 1 || strval(inputtext) > 100) return ShowPlayerDialog(playerid, 142, DIALOG_STYLE_INPUT, "Magazyn -> Step 2", "Podana iloœæ produktów jest nieprawid³owa, liczba musi mieœciæ siê w przedziale 1, 100.\n Podaj iloœæ produktów któr¹ zamierasz zakupiæ", "Ok", "Zamknij");
			new zapytanie[60];
...
public PrzelewBank(playerid)
{
  ...
  {
    new string1[100];
	format(string1, sizeof(string1), "Wpisz poni¿ej kwote, któr¹ chcesz przelaæ na konto %d:", pInfo[playerid][t_dialogtmp1]);
...
public OnPlayerCommandReceived(playerid, cmdtext[])
{
		...
		if( pInfo[playerid][bw] > 0 )
		{
		  ...
		  {
		      SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Podczas BW mo¿esz jedynie u¿ywaæ komendy /do, /report oraz /usmierc.");
              return 0;
		  } 
		}
		...
		{
		   SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Na Twoj¹ postaæ na³o¿ona jest blokada czatu ooc.");
		   return 0;
		}
        return 1;
}
...
public PlayerSpawnChangeList(playerid)
{
   ...
   {
      ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("B³¹d"), "Nie masz ¿adnych miejsc, w których mo¿esz siê spawnowaæ.", "Zamknij", "");
   }
   return 1;
...
public OnPlayerEditAttachedObject(playerid, response, index, modelid, boneid, Float:fOffsetX, Float:fOffsetY, Float:fOffsetZ, Float:fRotX, Float:fRotY, Float:fRotZ, Float:fScaleX, Float:fScaleY, Float:fScaleZ)
{
        if(!response)
        {
                ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Edycja obiektu", "Anulowaæ edycje obiektu", "OK", "");
                pInfo[playerid][EditorAttItem] = -1;
...
COMMAND:tel(playerid, params[])
{
   ...
   if( pInfo[playerid][lastUsedPhone] != -1 )
   {
      if( !isnull(params) )
      {
		  ...
		  {
		    SendClientMessage(playerid, WHITE, "{999A9C}PLERP.net: /tel [numer(aby zadzwoniæ)] (reszta opcji dostêpna w menu telefonu)");
		  }
		  else
		  {
		    if( numberTo == 911 )
			...
			if( numberTo == 777)
			{
				...
				if(pInfo[playerid][hotelOutdoor] != 0 ) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ.", "Zamknij", "");
				if(vw == 0 ) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ.", "Zamknij", "");
				new doorsid = GetDoorsByUid(vw);
				if(doors[doorsid][type] != 0) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ.", "Zamknij", "");
				new IDXGroup = GetGroupByUid(doors[doorsid][owner]);
				if(IDXGroup == -1) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ.", "Zamknij", "");
				new PlayerSlot = GetGroupPlayerSlot(playerid, IDXGroup);
				if(PlayerSlot == -1) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ.", "Zamknij", "");
				if(!HasPlayerPermission(playerid, "group", GPREM_storage, PlayerSlot)) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ", "Zamknij", "");
				SetPlayerSpecialAction(playerid, SPECIAL_ACTION_USECELLPHONE);
...
COMMAND:me(playerid, params[])
{
    ...
    if (isnull(params))
    {
       SendClientMessage(playerid, COLOR_GREY, "{999A9C}PLERP.net: /me [czynnoœæ]");
    }
    else
    {
...
COMMAND:wiadomosc(playerid, params[])
{
    ...
    if (isnull(params))
    {
       SendClientMessage(playerid, COLOR_GREY, "{999A9C}PLERP.net: /(w)iadomosc [id gracza] [treœæ]");
    }
    else
    {
	   new toId, sub[350];
       ...
       else
       {
         if( IsPlayerConnected(toId) )
	     {
	       ...
	       else SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz do którego próbujesz napisaæ nie jest zalogowany.");
	     }
	     else SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz do którego próbujesz napisaæ jest offline.");
       }
    }
    return 1;
}
...
COMMAND:oadd(playerid, params[])
{
	...
	else
	{
	  ...
	  {
	    new usedObjects = GetDoorsUsedObjects(doors[areaId][doorUid]);
	    if( doors[areaId][maxObjects]-usedObjects == 0 ) { SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Te drzwi osi¹gnê³y maksymaln¹ iloœæ obiektów."); return 1; }
	  }
...
COMMAND:odel(playerid, params[])
{
	...
	else
	{
      ...
      SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Obiekt o podanym ID zosta³ usuniêty.");
	}
...
COMMAND:osel(playerid, params[])
{
	...
	else
	{
	   ...
	   if( objectId == -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Brak obiektu o podanym modelu w odleg³oœci 10 metrów.");
...
COMMAND:ocopy(playerid, params[])
{
	...
	if( pInfo[playerid][editedObject] != -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby skopiowaæ obiekt, wy³¹cz tryb edycji obiektu.");
...
COMMAND:s(playerid, params[])
{
        ...
        } else
        {
                ...
                {
                        ShowPlayerDialog(playerid, 119, DIALOG_STYLE_INPUT, "Wpisz treœæ", "Wpisz w poni¿sze pole treœæ wiadomoœci, która ma byæ nadana na kanale radia Los Santos News:", "Akceptuj", "Anuluj");
                        pInfo[playerid][t_dialogtmp1] = 1;
                ...
                {
                        ShowPlayerDialog(playerid, 119, DIALOG_STYLE_INPUT, "Wpisz treœæ", "Wpisz w poni¿sze pole treœæ reklamy, która ma byæ nadana na kanale radia Los Santos News:", "Dalej", "Anuluj");
                        pInfo[playerid][t_dialogtmp1] = 5;
                } else if(!strcmp(option, "live", true))
                {
                        ShowPlayerDialog(playerid, 119, DIALOG_STYLE_INPUT, "Wywiad", "Wpisz ID gracza którym chcesz przeprowadziæ wywiad.", "Akceptuj", "Anuluj");
                ...
                } else if(!strcmp(option, "stoplive", true))
                {
                        ...
                        format(string, sizeof(string), "LSN: Zakoñczy³eœ wywiad na ¿ywo z %s.", pInfo[pid][name]);
                        SendClientMessage(playerid, COLOR_GREEN, string);
                        format(string, sizeof(string), "LSN: Dziennikarz %s zakoñczy³ wywiad na ¿ywo.", pInfo[playerid][name]);
                        SendClientMessage(playerid, COLOR_GREEN, string);
                }
...
COMMAND:drzwi(playerid, params[])
{
   ...
   if( areaId != -1 )
   {
	 if( HasPermissionToEditDoors(playerid, areaId) )
	 {
        ...
        format(doorsOpt, 300, "Zmieñ nazwê\nZmieñ op³atê za wejœcie\nZarz¹dzaj obiektami\n---------%s%s", autoClose, carsCrose);
   ...
   else
   {
      if( GetPlayerVirtualWorld(playerid) > 0 && pInfo[playerid][hotelOutdoor] == 0 )
      {
         ...
         format(doorsOpt, 400, "Zmieñ nazwê\nZmieñ op³atê za wejœcie\nZmieñ poyzcjê wejœciow¹\nZarz¹dzaj obiektami\n---------%s%s", autoClose, carsCrose);
...
COMMAND:stats(playerid, params[])
{
  ...
  else format( sexFormatted, 32, "mê¿czyzna" );
  
  format(options, 428, "[{ABABAB} Podstawowe informacje {FFFFFF}]\n Czas online\t\t\t{a9c4e4}%s\n Ostatnie logowanie\t\t{a9c4e4}%s\n P³eæ\t\t\t\t{a9c4e4}%s\n Gotówka\t\t\t{a9c4e4}$%d\n Bank\t\t\t\t{a9c4e4}$%d (%d)\n Podstawowy skin\t\t{a9c4e4}%d\n Aktywny skin\t\t\t{a9c4e4}%s\n[{ABABAB} Spawn {FFFFFF}]\n %s\n   {a9c4e4}zmieñ miejsce spawnu", timeOnlineF, lastLoginF, sexFormatted, pInfo[playerid][cash], pInfo[playerid][bankcash], pInfo[playerid][bank], pInfo[playerid][skin], activeSkin, whereSpawn);
...
COMMAND:oferuj(playerid, params[])
{
   ...
   else
   {
	 ...
	 if( !strcmp("pojazd", cmdParams[0], true) )
	 {
		...
		if( !IsPlayerInAnyVehicle(playerid) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby oferowaæ pojazd, musisz siê w nim znajdowaæ.");
		
		new vID = GetPlayerVehicleID(playerid);
		if( !CanEditCar(playerid, vID) || sVehInfo[vID][ownertype] != 0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie mo¿esz oferowaæ tego pojazdu.");
		...
		if( GetVehicleDistanceFromPoint(vID, bX, bY, bZ) > 10.0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz któremu sk³adasz ofertê znajduje siê zbyt daleko.");
	    ...
	    format(string2, sizeof(string2), "{f90012}Ta akcja wymaga potwierdzenia, poniewa¿ nie mo¿esz cofn¹æ jej sam.\n\
	                                     {a9c4e4}Czy napewno chcesz sprzedaæ pojazd %s o uid %d graczowi %s za kwotê $%d?\n\n\
										 {ffffff}Aby potwierdziæ swój wybór, wpisz w pole poni¿ej 'potwierdzam'.", VehicleNames[sVehInfo[vID][model]-400], sVehInfo[vID][uid], pInfo[offerTo][tdnick], offerPrice);
	    ShowPlayerDialog(playerid, 75, DIALOG_STYLE_INPUT, BuildGuiCaption("PotwierdŸ akcjê -> Sprzeda¿ pojazdu"), string2, "Dalej","Anuluj");
	 }
	 ...
	 if(!strcmp("ulecz", cmdParams[0], true))
	 {
		...
		if( GetPlayerDistanceFromPoint(playerid, pos[0], pos[1], pos[2]) > 10.0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz któremu sk³adasz ofertê znajduje siê zbyt daleko.");

		if(pInfo[playerid][currentDuty] == -1) return  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("Oferty -> B³¹d"), "Aby u¿yæ tej komendy musisz byæ na duty w s³u¿bie zdrowia.", "Zamknij", "");
		if(groups[pGroups[playerid][pInfo[playerid][currentDuty]][groupIndx]][type] != 7)return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("Oferty -> B³¹d"), "Aby u¿yæ tej komendy musisz byæ na duty w s³u¿bie zdrowia.", "Zamknij", "");
		
		new offerTo = strval(cmdParams[1]);
		new offerPrice = strval(cmdParams[2]);
		if(offerPrice < 100 || offerPrice > 300) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("Oferty -> B³¹d"), "Kwota oferty musi byæ w przedziale od 100$ do 300$.", "Zamknij", "");
		if(pInfo[offerTo][bwStatus] < 1) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("Oferty -> B³¹d"), "Nie mo¿esz oferowaæ leczenia temu graczowi, poniewa¿ ten nie jest os³abiony", "Zamknij", "");
		new Float:bX, Float:bY, Float:bZ;
		GetPlayerPos(offerTo, bX, bY, bZ);
		if(GetPlayerDistanceFromPoint(playerid, bX, bY, bZ) > 10.0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz któremu sk³adasz ofertê znajduje siê zbyt daleko.");
	 ...
	 if(!strcmp("podaj", cmdParams[0], true))
	 {
		...
		GetPlayerPos(strval(cmdParams[1]), pos[0], pos[1], pos[2]);
		if( GetPlayerDistanceFromPoint(playerid, pos[0], pos[1], pos[2]) > 10.0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz któremu sk³adasz ofertê znajduje siê zbyt daleko.");
		if(pInfo[playerid][hotelOutdoor] != 0 ) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie mo¿esz nic podaæ w tym miejscu.", "Zamknij", "");
		if(vw == 0 ) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie mo¿esz nic podaæ w tym miejscu.", "Zamknij", "");
		new doorsid = GetDoorsByUid(vw);
		if(doors[doorsid][type] != 0) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie mo¿esz nic podaæ w tym miejscu", "Zamknij", "");
		new IDXGroup = GetGroupByUid(doors[doorsid][owner]);
		if(IDXGroup == -1) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie mo¿esz nic podaæ w tym miejscu", "Zamknij", "");
		new PlayerSlot = GetGroupPlayerSlot(playerid, IDXGroup);
		if(PlayerSlot == -1) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ", "Zamknij", "");
		if(!HasPlayerPermission(playerid, "group", GPREM_offers, PlayerSlot)) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "Oferta -> B³¹d", "Nie posiadasz odpowiednich uprawnieñ", "Zamknij", "");
		...
		ShowPlayerDialog(playerid, 63, DIALOG_STYLE_LIST, "Oferta -> Podaj", bigstring, "Sprzedaj", "WyjdŸ");
		//Jestes chujem
	 ...
	 if(!strcmp("naprawe", cmdParams[0], true))
	 {
	   ...
	   if( GetPlayerVirtualWorld(playerid) > 0 && pInfo[playerid][hotelOutdoor] == 0)
	   {
		 ...
		 if( doors[doorsId][type] != 0 || groups[GetGroupByUid(doors[doorsId][owner])][type] != 3 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: W tym miejscu nie mo¿esz oferowaæ naprawy.");
	   }
	   else return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: W tym miejscu nie mo¿esz oferowaæ naprawy.");
	   ...
	   if( vID == -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz musi siedzieæ w pojeŸdzie, który chce naprawiæ.");
	   if( sVehInfo[vID][engine] == true ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Pojazd musi mieæ zgaszony silnik.");
	   ...
	   if( GetPlayerDistanceFromPoint(offerTo, sX, sY, sZ) > 15.0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz znajduje siê zbyt daleko.");
	 ...
	 if(!strcmp("malowanie", cmdParams[0], true))
	 {
	   ...
	   {
	     new doorsId = GetDoorsByUid(GetPlayerVirtualWorld(playerid));
		 if( doors[doorsId][type] != 0 || groups[GetGroupByUid(doors[doorsId][owner])][type] != 3 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: W tym miejscu nie mo¿esz oferowaæ malowania.");
	   }
	   else return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: W tym miejscu nie mo¿esz oferowaæ malowania.");
  ...
  else
  {
	...
	if( (!strcmp("g", offType, false) || !strcmp("grupa", offType, false)) )
	{
	  ...
	  else
	  {
		...
		{
          SendClientMessage(playerid, COLOR_GREY, "{FFB871}[B³¹d] {FFFFFF}Gracz, któremu próbujesz z³o¿yæ ofertê nie jest zalogowany.");
          return 1;
		}
		...
		if( groups[gIdx][type] == 3 )
		{
          if( !strcmp("naprawa", offerItem, false) )
		  {
			...
			{
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Oferty - B³¹d"), "Aby sk³adaæ oferty warsztatu musisz znajdywaæ siê w jego budynku.", "Zamknij", "");
		      return 1;
			}
			...
			{
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Oferty - B³¹d"), "Gracz, któremu sk³adasz ofertê naprawy nie znajduje siê w pojeŸdzie.", "Zamknij", "");
			  return 1;
			}
            ...
            {
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Oferty - B³¹d"), "Gracz, któremu sk³adasz ofertê jest zbyt daleko.", "Zamknij", "");
			  return 1;
            }
            if( sVehInfo[vehID][engine] == true )
            {
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Oferty - B³¹d"), "Pojazd w którym siedzi gracz ma odpalony silnik.", "Zamknij", "");
			  return 1;
			}
			...
			{
              ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, BuildGuiCaption("Oferty - B³¹d"), "Gracz, któremu sk³adasz ofertê naprawy siedzi w pe³ni sprawnym pojeŸdzie.", "Zamknij", "");
			  return 1;
			}
...
COMMAND:b(playerid, params[])
{
  ...
  else SendClientMessage(playerid, COLOR_GREY, "{999A9C}PLERP.net: /b [treœæ]");
...
COMMAND:k(playerid, params[])
{
  ...
  else SendClientMessage(playerid, COLOR_GREY, "{999A9C}PLERP.net: /k [treœæ]");
...
COMMAND:c(playerid, params[])
{
  ...
  else SendClientMessage(playerid, COLOR_GREY, "{999A9C}PLERP.net: /c [treœæ]");
...
COMMAND:kup(playerid, params[])
{
  ...
  if( doors[areaId][doorUid] == GetPlayerVirtualWorld(playerid) && doors[areaId][type] == 0 )
  {
	...
	if( groups[doorsGroup][type] == 5 || groups[doorsGroup][type] == 6 ) 
	{
		...
		{
		    liczba1++;
		    format( bigstring, sizeof(bigstring), "%s\n%d.\t\t%s[%d:%d] (Cena: $%d)(Iloœæ: %d)", bigstring, liczba1, Item[item][name], Item[item][value1], Item[item][value2], Item[item][price], Item[item][count]);
		...
		else ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "24/7 -> Kupowanie przedmiotu", "Przepraszamy, ale aktulanie brakuje przedmiotów w magazynie sklepu.", "Zamknij", "");
	}
...
{
   if( !IsPlayerInAnyDynamicArea(playerid) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby zatankowaæ pojazd musisz znajdowaæ siê na stacji paliw.");
   new area = GetPlayerDynamicArea(playerid);
   new grIndx = areas[area][owner]; 
   if( groups[grIndx][type] == 6 )
   {
      printf("test0");
      if( IsPlayerInAnyVehicle(playerid) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby zatankowaæ pojazd musisz z niego wysi¹œæ.");
      new iloscPaliwa;
      if( sscanf(params, "d", iloscPaliwa) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: /tankuj [iloœæ paliwa]");
	  else
	  {	 
	     new vehID = GetClosestVehicle(playerid);
	     printf("test1");
	     if( vehID == -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: W pobli¿u nie znajduje siê ¿aden pojazd.");
		 if( sVehInfo[vehID][engine] ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby zatankowaæ swój pojazd, musisz zgasiæ silnik.");
		 printf("test2");
   ...
   else return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Aby zatankowaæ pojazd musisz znajdowaæ siê na stacji paliw.");
...
{
	new Idx;
	if( pInfo[playerid][currentDuty] == -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie jesteœ na s³u¿bie, lub nie nale¿ysz do LSPD.");
	Idx = pGroups[playerid][pInfo[playerid][currentDuty]][groupIndx];
	if(groups[Idx][type] != 4) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie jesteœ na s³u¿bie, lub nie nale¿ysz do LSPD.");
...
COMMAND:taser(playerid, params[])
{
	...
	foreach(Player, i)
	{
		if(IsPlayerInRangeOfPoint(i, 5.0, Pos[0], Pos[1], Pos[2]) && i != playerid)
		{
			...
			format(string, sizeof(string), "Porazi³eœ taserem gracza %s.", pInfo[i][name]);
			SendClientMessage(playerid, COLOR_GREY, string);
			format(string, sizeof(string), "Zosta³eœ pora¿ony taserem przez funkcjonariusza %s.", pInfo[playerid][name]);
...
COMMAND:skuj(playerid, params[])
{
	...
	} else
	{
		...
		format(string, sizeof(string), "Za³o¿y³eœ kajdanki podejrzanemu %s.", pInfo[target][name]);
		SendClientMessage(playerid, COLOR_GREEN, string);
		format(string, sizeof(string), "Zosta³eœ skuty przez funkcjonariusza %s.", pInfo[playerid][name]);
...
COMMAND:odkuj(playerid, params[])
{
	...
	} else
	{
		...
		if(playerid == target) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie mo¿esz zdj¹æ sobie kajdanek.");
		
		SetPlayerSpecialAction(target, SPECIAL_ACTION_NONE);
		RemovePlayerAttachedObject(target, 0);
		pInfo[target][t_cuffed] = 0;
		
		format(string, sizeof(string), "Zdj¹³eœ kajdanki podejrzanemu %s.", pInfo[target][name]);
		SendClientMessage(playerid, COLOR_RED, string);
	...
	} else
	{
		if(!strcmp(option, "naloz", true))
		{
			ShowPlayerDialog(playerid, 93, DIALOG_STYLE_INPUT, "Blokada pojazdu", "Wpisz dane w formacie\n\
																					Rejestracja pojazdu;Kwota zdjêcia blokady;Powód", "Akceptuj", "Anuluj");
		} else if(!strcmp(option, "zdejmij", true))
		{
			ShowPlayerDialog(playerid, 94, DIALOG_STYLE_INPUT, "Blokada pojazdu", "Wpisz rejestracjê pojazdu z którego chcesz zdj¹æ blokadê.", "Akceptuj", "Anuluj");
		}
	}
	return 1;
}
...
COMMAND:usmierc(playerid, params[])
{
        if(pInfo[playerid][bw] > 0)
        {
                ShowPlayerDialog(playerid, 95, DIALOG_STYLE_MSGBOX, "Uœmiercenie postaci", "Uœmiercenie postaci spowoduje brak mo¿liwoœci ponownego zalogowania siê na dan¹ postaæ.\nUœmierciæ postaæ?", "Tak", "Nie");
        } else
        {
                return 1;
        }
...
COMMAND:pokoj(playerid, params[])
{
   ...
   else
   {
	  ...
	  if( !strcmp("wymelduj", params, true) )
	  {
		...
		else return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w hotelu.");

		if( areaId == -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w hotelu.");

		if( doors[areaId][type] != 2 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w hotelu.");

		if( !Iter_Contains(Doors_Renters[areaId], pInfo[playerid][uid]) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie jesteœ zameldowany w tym hotelu.");
		...
		return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Wymeldowa³eœ siê z hotelu.");
	  }
	  ...
	  if( !strcmp("zamelduj", params, true) )
	  {
		...
		if( Iter_Contains(Doors_Renters[areaId], pInfo[playerid][uid]) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Jesteœ ju¿ zameldowany w tym hotelu.");
		...
		return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Zameldowa³eœ siê w hotelu.");
	  }
  ...
  if( GetPlayerVirtualWorld(playerid) > 0 && pInfo[playerid][hotelOutdoor] == 0 )
  {
	...
	if( doors[doorsId][type] == 0 && groups[GetGroupByUid(doors[doorsId][owner])][type] == 10 )
	{
	   if( pInfo[playerid][bank] == 0 )
	   {
		  ...
		  format( string1, sizeof(string1), "Witaj w g³ównym banku Los Santos!\n\n\
		                                     Nie posiadasz jeszcze swojego konta bankowego, ale nie przejmuj siê\n\
											 poniewa¿ jego utworzenie zajmie Ci tylko kilka chwil.\n\n\
											 Jakie korzyœci daje Ci w³asne konto bankowe?\n\
											 \t - bezpieczeñstwo pieniêdzy w banku\n" );
		  format( string2, sizeof(string2), "\t - ³atwoœæ w dokonywaniu transkacji miêdzy kontami\n\
											 \t - dostêp do gotówki praktycznie w ca³ym los santos dziêki bankomatom\n\n\
											 Aby za³o¿yæ swoje konto naciœnij poni¿szy przycisk 'Za³ó¿'.");
		  strcat(string1, string2);
	      ShowPlayerDialog(playerid, 121, DIALOG_STYLE_MSGBOX, "Bank » Zak³adanie konta [1/2]", string1, "Za³ó¿", "Anuluj");
	   }
       else
	   {
	      pInfo[playerid][t_dialogtmp3] = 1;
	...
	else SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w banku.");
  }
  else SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w banku.");
  return 1;
}
...
COMMAND:bankomat(playerid, params[])
{
  ...
  if( areaId == -1 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w pobli¿u bankomatu.");
...
COMMAND:plac(playerid, params[])
{
  ...
  if( payAmount > pInfo[playerid][cash] ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie masz tyle pieniêdzy.");
  
  if( payTo == playerid ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie mo¿esz p³aciæ samemu sobie.");
  if( !IsPlayerConnected(payTo) || pInfo[payTo][logged] == 0 ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz, któremu chcesz zap³aciæ jest nie zalogowany.");
  
  new Float:ppX, Float:ppY, Float:ppZ;
  GetPlayerPos(playerid, ppX, ppY, ppZ);
  if( !IsPlayerInRangeOfPoint(payTo, 5.0, ppX, ppY, ppZ) || GetPlayerVirtualWorld(payTo) != GetPlayerVirtualWorld(playerid) ) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Gracz, któremu chcesz zap³aciæ jest za daleko.");
  
  new meMSG[100];
  format(meMSG, sizeof(meMSG), "wyci¹ga trochê pieniêdzy i podaje je %s", pInfo[payTo][name]);
...
{
	if(GetPlayerVirtualWorld(playerid) != 0) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie mo¿esz tutaj u¿yæ tej komendy.");
	...
	if(currbus == -1) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w pobli¿u ¿adnego przystanku.");
	if(strval(param))
	{
		...
		if(pInfo[playerid][cash] < floatround(odleglosc*BUS_CENA, floatround_ceil) && pInfo[playerid][timeOnline] > 18000) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nieposiadasz pieniêdzy na bilet.");
	...
	{	
		ShowPlayerDialog(playerid, 42, DIALOG_STYLE_LIST, "Wybierz przystanek", string, "Ok", "WyjdŸ");
	}
...
public CheckBanQuery(playerid)
{
        ...
        format(string, sizeof(string), "Twoja postaæ jest zbanowana. Powód: %s", reason);
        SendClientMessage(playerid, COLOR_RED, string);
...
public CheckWarnQuery(playerid)
{
        ...
        if(rows >= 3)
        {
                format(string, sizeof(string), "Twoja postaæ otrzyma³a ostatnio 3 warny, nie mo¿esz na niej graæ.");
                SendClientMessage(playerid, COLOR_RED, string);
                defer KickTimer[500](playerid);
        }
        return 1;
...
COMMAND:wydaj(playerid, params[])
{
        ...
        } else
        {
                ...
                {
                        ShowPlayerDialog(playerid, 155, DIALOG_STYLE_INPUT, "Wyrób tablice rejestracyjne", "Wpisz dane w formacie:\n\
...
public CheckAdminJailQuery(playerid)
{
        ...
        SendClientMessage(playerid, COLOR_RED, "PLERP.net: Wróci³eœ do Admin Jaila. Poczekaj a¿ twoja kara siê skoñczy.");
        SetPlayerVirtualWorld(playerid, pInfo[playerid][uid]);
...
COMMAND:magazyn(playerid)
{
		...
		if(pInfo[playerid][currentDuty] == -1) return  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("Oferty -> B³¹d"), "Aby u¿yæ tej komendy musisz byæ na duty w firmie transportowej.", "Zamknij", "");
		if(groups[pGroups[playerid][pInfo[playerid][currentDuty]][groupIndx]][type] != 14) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("B³¹d"), "Aby u¿yæ tej komendy musisz byæ na duty w firmie transportowej.", "Zamknij", "");
		if(GetPlayerDistanceFromPoint(playerid, 1644.54, 1068.87, 10.82) > 10.0 ) 
		{
			if(IsValidDynamicCP(Dostawa[playerid][0])) return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Masz ju¿ w³¹czone namierzanie. Aby je wy³¹czyæ u¿yj /magazyn.");
			...
			return SendClientMessage(playerid, COLOR_GREY, "PLERP.net: Nie znajdujesz siê w pobli¿u magazynu, zosta³ on zaznaczony na mapie.");
		}
		if(Dostawa[playerid][2])return  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX,("Oferty -> B³¹d"), "Nie mo¿esz teraz u¿yæ tej komendy.", "Zamknij", "");
...
public MagazynOdbierz(playerid, GROUP)
{
	...
	for (new i; i<rows; i++)
	{
		...
		ShowPlayerDialog(playerid, 144, DIALOG_STYLE_LIST, "Magazyn -> Odbiór", bigstring, "Wybierz", "Zamknij");
	return 1;
}
...
public GdzieDos(playerid)
{
	...
	if(!rows) return ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "B³¹d", "Wyst¹pi³ b³¹d.", "ZAMKNIJ", "");
		...
		{
			if(Dostawa[gracz][3] == doorsid) return  ShowPlayerDialog(playerid, 1, DIALOG_STYLE_MSGBOX, "B³¹d", "Ktoœ ju¿ dostarcza t¹ paczke.", "ZAMKNIJ", "");
		}
